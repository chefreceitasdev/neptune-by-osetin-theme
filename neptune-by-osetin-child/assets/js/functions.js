(function($) {
    ...
    $(function() {
        ...
        function printdiv(printdivname) {
            var headstr = "<html><head><title>Detalhes da Receita</title></head><body>";
            var footstr = "</body>";
            var newstr = document.getElementById(printdivname).innerHTML;
            //var newmedia = document.getElementsByClassName('single-main-media')[0].innerHTML;
            var newmedia = document.getElementsByClassName('single-main-media-image-w')[0].innerHTML;
            var oldstr = document.body.innerHTML;
            document.body.innerHTML = headstr + newmedia + newstr + footstr;
            window.print();
        ...
        /// COOKING MODE
        /// ------------------
        /// Editada por Diego Fernandes 15/02/2022

        $('body').on('click', '.cooking-mode-toggler', function() {
            if ($('.cooking-mode-w').length) {

                $("#single-receita-print").removeClass("single-receita-cooking-mode");

                $('.single-content').html($('.cooking-mode-i').html());
                $('.cooking-mode-w').fadeOut('slow', function() { $('.cooking-mode-w').remove(); });
            } else {

                $("#single-receita-print").addClass("single-receita-cooking-mode");

                var recipe_content = $('.single-content').html();
                //var recipe_content = $('.body > div.all-wrapper.with-animations > div.single-panel.os-container').html();
        ...
        // Tooltip RATINGS Modificado 05/02/2022 Diego

        $('.tooltip-trigger-rattings').on({
            mouseenter: function() {
                var tooltip_header = $(this).data('tooltip-header');
                $(this).append('<div class="tooltip-box-rattings"><div class="tooltip-box-header">' + tooltip_header + '</div></div>');
            },
            mouseleave: function() {
                $(this).find('.tooltip-box-rattings').remove();
            }
        });
