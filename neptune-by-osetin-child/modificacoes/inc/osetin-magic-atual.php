...
function output_recipe_media($post_id = false, $image_size = 'osetin-full-width'){
  ...
  if(('video' == get_post_format()) && osetin_get_field('video_shortcode')){
    ...
    if(pathinfo($img_src, PATHINFO_EXTENSION) == 'gif' && osetin_get_field('is_gif_media')){
      ...
      $gif_html = '<span class="gif-label"><i class="os-icon os-icon-basic1-082_movie_video_camera"></i><span>'.__('GIF', 'neptune-by-osetin').'</span></span>';
...
function osetin_generate_recipe_rich_snippet(){
    ...
    $cuisines = get_the_terms( $recipe_post_id, 'recipe_cuisine' );
    if ( !empty($cuisines) && ! is_wp_error( $cuisines ) ) {
      $cuisines_names = [];
      foreach($cuisines as $cuisine){
        $cuisines_names[] = $cuisine->name;
      }
      $google_meta['recipeCuisine'] = implode(',', $cuisines_names);
    }

    $categories = get_the_category($recipe_post_id);
    if ( !empty($categories) && ! is_wp_error( $categories ) ) {
      $categories_names = [];
      foreach($categories as $category){
        $categories_names[] = $category->name;
      }
      $google_meta['recipeCategory'] = implode(',', $categories_names);
    }

    $tags = get_the_tags($recipe_post_id);
    if ( !empty($tags) && ! is_wp_error( $tags ) ) {
      $tags_names = [];
      foreach($tags as $tag){
        $tags_names[] = $tag->name;
      }
      $google_meta['keywords'] = implode(',', $tags_names);
    }
    ...
    if($recipe_reviews){
      ...
      if ( $osetin_reviews_query->have_posts() ) {
        while ( $osetin_reviews_query->have_posts() ) : $osetin_reviews_query->the_post();
          ...
          // Editado 25/02/2022 Diego Fernandes. - Tira as Tags HTML do reviewBody
          $review_info['reviewBody'] = strip_tags(osetin_get_field('body'));
    ...
    // Modificação na função original para melhorar o SEO
    // Editada 22/01/2022
    ...
    if($prepTime) {
      $google_meta['prepTime'] = $prepTime;
    }
    else {
      $google_meta['prepTime'] = osetin_get_field('recipe_preparation_time', $recipe_post_id);
    }

    if($cookTime) {
      $google_meta['cookTime'] = $cookTime;
    }
    else {
      $google_meta['cookTime'] = osetin_get_field('recipe_just_cooking_time', $recipe_post_id);
    }

    if($totalTime) {
      $google_meta['totalTime'] = $totalTime;
    }
    else {
     // Modificado para o import do Allrecipes 21/01/2022
     $google_meta['totalTime'] = osetin_get_field('recipe_preparation_time', $recipe_post_id); // same: recipe_cooking_time
     #recipe_cooking_time
    }

    // add integer Yield
    // Editada em Janeiro/2022
    $google_meta_serves = (int)osetin_get_field('recipe_serves', $recipe_post_id);
    $google_meta['recipeYield'] = ($google_meta_serves) ? $google_meta_serves : '';
    // INGREDIENTS
  ...
  /*   if( have_rows('steps', $recipe_post_id) ){
    ...
    if(!empty($google_meta)){
     ...
     // STEPS 
     //
     // Editado 25/02/2022 adicionado HowToStep elementos schema.org recipe
     //
     ...
     if( have_rows('steps', $recipe_post_id) ){
      ...
      while ( have_rows('steps', $recipe_post_id) ) { the_row();
        ...
        $step_name = strip_tags(get_sub_field('step_title'));

        if (!$step_name) {
          $step_name = strip_tags(get_sub_field('step_description'));
        }

        //blog-recipe-' . get_the_ID() . '-step-0-' . $step_number;
        $step_url = get_the_permalink( $recipe_post_id );
        $step_url .= '#blog-recipe-' . $recipe_post_id . '-step-0-' . $step_number;

        $step_number++;

        $metadata_instruction = array(
          '@type' => 'HowToStep',
          'text' => strip_tags(get_sub_field('step_description')),
          'name' => $step_name,
          'url' => $step_url,
        );

        array_push($google_meta['recipeInstructions'], $metadata_instruction );
      }
    }
    if(!empty($google_meta)){
      return '<script id="chefreceitasJSONLD"type="application/ld+json">' . json_encode($google_meta) . '</script>';
    }else{
      return '';
    }
}
...
// Edit: 21/01/2022 Diego 
/* onclick="window.open('https://www.facebook.com/sharer.php?u=<?php echo $post_link; ?>', 'sharer', 'toolbar=0,status=0,width=620,height=280');"

<a itemprop="url" class="chefreceitas-share-link" href="#" onclick="window.open('https://www.facebook.com/sharer.php?u=https://newchefreceitas.local/recipe/picadinho-de-frutos-do-mar-do-litinho/', 'sharer', 'toolbar=0,status=0,width=620,height=280');">
...
function osetin_get_sharing_icons(){
  ...
  ?>
  <a itemprop="url" href="#" class="archive-item-share-link aisl-facebook" onclick="window.open('<?php echo esc_url($facebook_share_link); ?>', 'sharer', 'toolbar=0,status=0,width=620,height=280');"><i class="os-icon os-icon-facebook"></i></a>
...
// editada 03/03/2022 add $recipe_title = focus keyfrase

// NOVA FUNÇÃO COM JANELA POPUP
function osetin_get_sharing_icons(){
  $id_post = get_the_ID();
  //$recipe_title = get_the_title();
  $recipe_title = get_post_meta( $id_post, '_yoast_wpseo_focusk??w', true );
  $sharing_url = get_the_permalink();
  $img_to_pin = has_post_thumbnail() ? wp_get_attachment_url( get_post_thumbnail_id() ) : "";
  $osetin_current_title = is_front_page() ? get_bloginfo('description') : wp_title('', false);
  ...
  <a href="#share-zap-<?php echo $id_post; ?>" id="#share-zap-<?php echo $id_post; ?>" class="archive-item-share-link aisl-mail" onclick="popUp=window.open('https://api.whatsapp.com/send?phone=&amp;text=Aprenda a preparar a deliciosa Receita de %2A<?php echo urlencode($recipe_title); ?>%2A com o Chef Receitas Blog%3A%0A<?php echo esc_url($sharing_url); ?>', 'popupwindow', 'scrollbars=yes,width=800,height=400');popUp.focus();return false;"><i class="fa fa-whatsapp" aria-hidden="true" style="color: #ff6666;"></i></a>
                    ...
                    <div class="featured-recipe-details-w">
                      <div class="featured-recipe-cooking-time">
                        <?php if(!empty($cooking_time)){ ?>
                          <?php esc_html_e('Cooking Time:', 'neptune-by-osetin'); ?> <?php echo osetin_get_field('recipe_cooking_time'); ?>
                        <?php }else{ ?>
                          <?php echo get_the_category_list(); ?>
                        <?php } ?>
                      </div>
...
function osetin_get_difficulty_string($number){
  $difficulties = array('1' => __('Easy', 'neptune-by-osetin'), '2' => __('Medium', 'neptune-by-osetin'), '3' => __('Hard', 'neptune-by-osetin'));
  if($number){
  ...
  <?php }else{ ?>
    <?php if(get_next_posts_link() || get_previous_posts_link()){ ?>
      <div class="archive-pagination classic-pagination">
        <div class="archive-pagination-prev"><?php previous_posts_link( esc_html__('Previous Entries', 'neptune-by-osetin') ); ?></div>
        <div class="archive-pagination-next"><?php next_posts_link( esc_html__('Next Entries', 'neptune-by-osetin'), '' ); ?></div>
      </div>
    <?php } ?>
  <?php }
}
...
function osetin_excerpt_more($more_link_class = 'read-more-link') {
  if(get_post_format(get_the_ID()) == 'link'){
    return '...<div class="'.$more_link_class.'"><a href="'. osetin_get_field( 'external_link' ) . '">' . esc_html__('Read More', 'neptune-by-osetin') . '</a></div>';
  }else{
    return '...<div class="'.$more_link_class.'"><a href="'. get_permalink( get_the_ID() ) . '">' . esc_html__('Read More', 'neptune-by-osetin') . '</a></div>';
  }
}
...
function osetin_get_post_thumbnail($post_id, $thumbnail_name = 'osetin-medium-square-thumbnail'){
  if(has_post_thumbnail($post_id)){
    ...
    return '<img src="'.$placeholder_url.'" alt="'.__('Featured Image', 'neptune-by-osetin').'"/>';
  }
}
 ...
 * 
 */
// EDITADA 21/01/2022 DIEGO
// 03/03/2022 add $recipe_title = focus keyfrase  (titulo original)
...
function osetin_post_share_box(){
  ...
  <div class="post-share-box">
    ...
    <h3 class="post-share-header"><?php esc_html_e('Share it on your social network:', 'neptune-by-osetin'); ?></h3>
    <div class="psb-links">
      <a itemprop="url" href="#" class="psb-link psb-facebook" onclick="window.open('<?php echo esc_url($facebook_share_link); ?>', 'sharer', 'toolbar=0,status=0,width=620,height=280');"><i class="os-icon os-icon-facebook"></i></a>
      ...
      <a href="#" class="psb-link psb-mail" style="background-color:green;position:relative;top:-3px;" onclick="popUp=window.open('https://api.whatsapp.com/send?phone=&amp;text=Aprenda a preparar a deliciosa Receita de %2A<?php echo urlencode($recipe_title); ?>%2A com o Chef Receitas Blog%3A%0A<?php echo esc_url($sharing_url); ?>', 'popupwindow', 'scrollbars=yes,width=800,height=400');popUp.focus();return false;"><i class="fa fa-whatsapp" aria-hidden="true" style="color: #ffffff;"></i></a>
    ...
    <div class="psb-url">
      <div class="psb-url-heading"><?php esc_html_e('Or you can just copy and share this url', 'neptune-by-osetin'); ?></div>
...
function osetin_output_breadcrumbs(){
  if(osetin_is_bbpress()){
    ...
    echo '<ul class="bar-breadcrumbs">';
      if(is_home()){
        echo '<li>'.esc_html__('Home', 'neptune-by-osetin').'</li>';
      }elseif(is_category()){
        echo '<li><a href="'.site_url().'">'.esc_html__('Home', 'neptune-by-osetin').'</a></li>';
        ...
        echo '<li><a href="'.site_url().'">'.esc_html__('Home', 'neptune-by-osetin').'</a></li>';
        echo '<li><a href="'.get_user_meal_plans_permalink().'">'.esc_html__('My Meal Plans', 'neptune-by-osetin').'</a></li>';
        ...
        if(!empty($categories)){
          ...
          echo '<li><a href="'.get_category_link( $category->term_id ).'" title="' . esc_attr( sprintf( esc_html__( "View all posts in %s", 'neptune-by-osetin' ), $category->name ) ) . '">'.$category->cat_name.'</a></li>';
        }
        echo '<li>'.get_the_title().'</li>';
      }
    echo '</ul>';
...
/* function osetin_get_post_sharing_icons(){
  ...
  <span><?php _e('Share', 'neptune-by-osetin'); ?></span>
...
/* function osetin_get_post_sharing_icons(){
  ...
  <span><?php _e('Share', 'neptune-by-osetin'); ?>
</span>
