...
?>
  <?php  //#osetinRecipeReviews MODIFICA PARA O LINK ABRANGER TODO A div ratings
        ...
        if(!in_array('rating', $hidden_elements_array)){
          ...
          if($reviews_info){
            ...
            if((int)$reviews_info->total_reviews >= 2) {
              $avaliacao_texto = "$reviews_info->total_reviews Avaliações";
            }
            else {
              $avaliacao_texto = "$reviews_info->total_reviews Avaliação";
            }

            echo '<div class="archive-item-rating-thumbnail-top tooltip-trigger-rattings" data-tooltip-header="'. $avaliacao_texto .'" style="width: 100%;">';
            
            if($reviews_info->avg_rating) {

              // adicionada o link na DIV inteira
              if($reviews_info->total_reviews) echo '<a href="'.get_permalink().'#osetinRecipeReviews" class="review-summary-total">';
              ...
              //if($reviews_info->total_reviews) echo '<span class="review-summary-total">'.$reviews_info->total_reviews.'</span>';
              echo '</div></a>';
            }
            echo '</div>';
          } 
          else {
            //AVALIE ESTA RECEITA, CLIQUE AQUI
            // IMPLEMENTADA 12/02/2022
            // REDIRECIONA PARA post single recipe na sessão de reviews. Formulário de Acesso senão tiver logado
            $div_stars = '<div id="receita-sem-reviews" class="review-stars-w"><i class="os-icon os-icon-star-full rating-star-on"></i><i class="os-icon os-icon-star-full rating-star-on"></i><i class="os-icon os-icon-star-full rating-star-on"></i><i class="os-icon os-icon-star-full rating-star-on"></i><i class="os-icon os-icon-star-full rating-star-on"></i></div>';

            $reviews_chain = '<a href="'.get_permalink().'#osetinRecipeReviews" class="review-summary-total avalie-receita-link">';
            echo '<div class="archive-item-rating-thumbnail-top tooltip-trigger-rattings" data-tooltip-header="Avalie a receita!" style="width: 100%;">' .  $reviews_chain . $div_stars . '</a></div>';
          }         
        }
      ?>
<article <?php post_class('archive-item any '.$current_step_class); ?>>  
  
  <div class="archive-item-i" id="shareiconSection">
    <div class="extra-styling-box"></div>
    <?php if(!in_array('share', $hidden_elements_array)){ ?>
      <div class="archive-item-share-w active">
        <div class="archive-item-share-trigger">
          <div class="archive-item-share-plus"><i class="os-icon os-icon-plus"></i></div>
          <div class="archive-item-share-label"><?php esc_html_e('Share', 'neptune-by-osetin'); ?></div>
    ...
    <div class="archive-item-media">
       <!--  ADICIONA TEXTO EM CIMA DE CADA FOTO   -->
    ...
    <div class="archive-item-content">
          ...
          <?php if(!in_array('cooking_time', $hidden_elements_array)){ ?>
            ...
            <?php if(!empty($cooking_time)){ ?>
              <div class="archive-item-meta-cooking-time">
                <?php esc_html_e('Cooking Time: ', 'neptune-by-osetin'); ?><?php echo esc_html($cooking_time); ?>
              </div>
            <?php } ?>  
          <?php } ?>
      ...
      <?php if($current_step_class != 'full_third'){ ?>
        <!--   EDITADA PARA NAO APARECE O LIKE NESTA LINHA 05/02/2022   -->
        <?php //if(array_diff(array('author_avatar', 'author_name', 'date', 'like'), $hidden_elements_array)){ ?>
        
        <?php if(array_diff(array('author_avatar', 'author_name', 'date'), $hidden_elements_array)){ ?>
          <div class="archive-item-author-meta">

            <!--   EDITADO EM 05/02/2022 MUDANÇAS NA FORMA QUE APARECE  -->
      ...
      <!--   EDITADA PARA APARECER O LIKE NESTA LINHA 05/02/2022   -->
      <?php //if(array_diff(array('rating', 'read_more', 'comments', 'views'), $hidden_elements_array)){ ?>

      <?php if(array_diff(array('rating', 'read_more', 'comments', 'views', 'like'), $hidden_elements_array)){ ?>
        <div class="archive-item-deep-meta">
           <!--  AQUI FICAVA O RATINGS  -->
          <?php if(!in_array('comments', $hidden_elements_array)){ ?>
            <div class="archive-item-comments">
              <a href="<?php the_permalink() ?>#singlePostComments"><i class="os-icon os-icon-thin-comment"></i> <span><?php comments_number( __('Comment', 'neptune-by-osetin'), '1 '.__('Comment', 'neptune-by-osetin'), '% '.__('Comments', 'neptune-by-osetin') ); ?></span></a>
            </div>
          <?php } ?>
          <?php if(!in_array('views', $hidden_elements_array)){ ?>
            <?php if(function_exists('echo_tptn_post_count')){ 
              ...
              echo '<i class="os-icon os-icon-eye"></i> <span>'. do_shortcode('[tptn_views daily="0"]'). ' ' . esc_html__('Views', 'neptune-by-osetin') .'</span>';              
              echo '</div>'; ?>
            <?php } ?>
          <?php } ?>

          <!--   EDITADO EM 05/02/2022 ADICIONA BOTAO CURTIR  -->
          <?php if(!in_array('like', $hidden_elements_array)){ ?>
              <div class="archive-item-views-count-likes">
                <?php osetin_vote_build_button(get_the_ID(), 'slide-button slide-like-button'); ?>
              </div>
          ...
          <?php if(array_diff(array('rating', 'read_more'), $hidden_elements_array)){ ?>
            <div class="archive-item-rating-and-read-more" style="width: 100%;">
              <?php  //#osetinRecipeReviews MODIFICA PARA O LINK ABRANGER TODO A div ratings
              /*   if(!in_array('rating', $hidden_elements_array)){
                  $reviews_info = osetin_recipe_rating_average_and_total(get_the_ID()); 
                  if($reviews_info){
                    if($reviews_info->avg_rating) {
                      ...
                      if($reviews_info->total_reviews) echo '<a href="'.get_permalink().'#osetinRecipeReviews" class="review-summary-total">';
                      echo '<div class="archive-item-rating">';
                      // adicionado o texto Avaliações
                      if((int)$reviews_info->total_reviews >= 2) {
                          echo '<span class="archive-item-rating-avaliacoes">  Avaliações</span>';
                      }
                      else {
                        echo '<span class="archive-item-rating-avaliacoes">  Avaliação</span>';
                      }
              ...
              <?php if(!in_array('read_more', $hidden_elements_array) && $current_step_class != 'full_third'){ ?>
                <div class="archive-item-read-more-btn">
                  <div class="read-more-link"><a href="<?php the_permalink(); ?>"><?php _e('Read More', 'neptune-by-osetin'); ?></a></div>
                </div>
              <?php } ?>
            </div>
          <?php } ?>
    ...
    <?php } ?>
    <div class="clear"></div>
    <?php  // edit_post_link( esc_html__( 'Edit', 'neptune-by-osetin' ), '<span class="edit-link">', '</span>' ); ?>
  </div>
</article>